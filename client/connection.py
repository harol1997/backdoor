...
class Connector(socket):
    ...
    def recv_str(self, conn:socket=None)->None:
        """Print string from socket

        :param conn: object socket, defaults to None
        :type conn: socket, optional
        """
        for data in self.recv_bytes(con=conn):
            try:
    ...
    def get_str(self, conn:socket=None)->str:
        """
        :param conn: socket object, defaults to None
        :type conn: socket, optional
        :return: string received in socket
        :rtype: str
        """
        string = ""
        for data in self.recv_bytes(con=conn):
